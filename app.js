...
const state = {
  ...
  filterTeam: 'Todas as equipes', refDate: today(), saved: false, form: emptyForm(), formTab:'identificacao', editing:null,dirty:false,saving:false,authBusy:false,recordsLoading:false,recordsError:'',teams:[],defaults:null,groups:[],groupsTeam:'',groupsVersion:'',preferencesBusy:false,notice:'',securityUsers:[],securityEvents:[],securityCount:0,securityOffset:0,securityLoading:false,securityBusy:false,securityError:''
};
...
function resetSession(){
  ...
  Object.assign(state,{user:null,profile:null,authPass:'',authEmail:'',authName:'',authTeam:'',
    ...
    saving:false,authBusy:false,recordsLoading:false,recordsError:'',teams:[],defaults:null,groups:[],groupsTeam:'',groupsVersion:'',preferencesBusy:false,notice:'',securityUsers:[],securityEvents:[],securityCount:0,securityOffset:0,securityLoading:false,securityBusy:false,securityError:'',filterTurn:'Todos',
...
async function saveRecord(){
  ...
  try{
    ...
    if(version!==sessionVersion)return;
    state.form.assinatura=data.assinatura;state.editing=data;state.dirty=JSON.stringify({...state.form,assinatura:undefined})!==JSON.stringify({...JSON.parse(snapshot),assinatura:undefined});state.saved=!state.dirty;
    await loadRecords();
  }catch(error){if(version===sessionVersion)state.error=error.message}
...
function openRecord(id){
  ...
  state.form.assinatura=row.assinatura;state.editing={id:row.id,updated_at:row.updated_at};state.dirty=false;state.saved=false;state.error='';state.view='form';render();
}
async function deleteRecord(id){
  if(state.saving)return;
  const row=state.records.find(r=>r.id===id);if(!row)return;
  if(!confirm(`Excluir o plantao de ${shortDate(row.data)} - ${row.turno} - ${row.equipe||'sem equipe'}?`))return;
  const version=sessionVersion;
...
function formTabNav(){const f=state.form;return `<div class="form-tabs no-print" role="tablist">${tabButton('identificacao','Identificação')}${tabButton('equipe','Equipe',(f.integrantes||[]).filter(x=>String(x).trim()).length)}${tabButton('demandas','Demandas',(f.dia||[]).filter(x=>String(x).trim()).length)}${tabButton('repasses','Repasses',(f.proximo||[]).filter(x=>String(x).trim()).length)}${tabButton('ocorrencias','Ocorrências',(f.ocorrencias||[]).filter(o=>String(o.texto||'').trim()).length)}</div>`}
function compactFormView(){
  ...
  const saveBar=`<div class="save-strip"><div><strong>${state.saved?'Plant�o salvo':'Pronto para salvar'}</strong><span>${state.dirty?'H� altera��es neste plant�o.':'As informa��es atuais est�o sincronizadas.'}</span></div><button id="save-record" class="primary" ${state.saving?'disabled':''} onclick="saveRecord()">${state.saving?'Salvando...':state.saved?'Salvar novamente':'Salvar plant�o'}</button></div>`;
...
document.addEventListener('click',event=>{const open=event.target.closest('[data-open-record]');if(open)openRecord(open.dataset.openRecord);const del=event.target.closest('[data-delete-record]');if(del)void deleteRecord(del.dataset.deleteRecord)});
