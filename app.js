...
function compactFormView(){
  ...
  const saveBar=`<div class="save-strip"><div><strong>${state.saved?'Plantão salvo':'Pronto para salvar'}</strong><span>${state.dirty?'Há alterações neste plantão.':'As informações atuais estão sincronizadas.'}</span></div><button id="save-record" class="primary" ${state.saving?'disabled':''} onclick="saveRecord()">${state.saving?'Salvando...':state.saved?'Salvar novamente':'Salvar plantão'}</button></div>`;
