...
function groupsView(){const ready=state.groupsTeam===state.form.equipe;return `<div class="group-panel"><label>Grupos da equipe</label><div class="row"><button type="button" onclick="loadGroups()">Carregar grupos</button>${ready?`<select aria-label="Aplicar grupo" onchange="if(this.value!=='')applyGroup(Number(this.value))"><option value="">Selecione um grupo</option>${state.groups.map((g,i)=>`<option value="${i}">${esc(g.nome)} · ${g.integrantes.length} integrantes</option>`).join('')}</select>`:''}</div><p class="hint">Carregue um grupo para preencher todos os integrantes de uma vez.</p>${ready?`<div class="row"><input id="group-name" maxlength="120" aria-label="Nome do grupo" placeholder="Nome do grupo, ex.: Plantão diurno"><button ${state.preferencesBusy?'disabled':''} onclick="saveGroup()">Salvar integrantes como grupo</button></div>`:''}</div>`}
...
function render(){const app=document.getElementById('app');if(state.loading)app.innerHTML='<main class="wrap"><p>Carregando...</p></main>';else if(serverUnavailable)app.innerHTML=setupView();else if(!state.user)app.innerHTML=authView();else if(!state.profile)app.innerHTML=`<main class="wrap"><p>${esc(state.error||'Acesso indisponivel.')}</p><button onclick="logout()">Sair</button></main>`;else if(state.view==='security'&&isChief())app.innerHTML=securityView();else if(state.view==='report')app.innerHTML=reportView();else if(state.view==='consolidado')app.innerHTML=consolidatedView();else app.innerHTML=formView()}
